...
![001.png](img/001.png)
* ( слайд вставлен взамен ошибочного ). 
* На данном слайде отображена уже измененная конфигурация для добавления опций к запускаемому процессу через внешний файл.
...
`ExecStart=/usr/sbin/node_exporter` (через запуск команды `sysmetctl edit unit`), путем добавления к нему требуемой опции.  
Но в задании предложено взять за основу схему добавления опций в cron, при помощи определения `EnvironmentFile=`, тем более,
что она имеет более высокий приоритет перед переопредением параметров юнитов при помощи `sysmetctl edit unit`. 

Реультат ( после применения опции для смены порта метрик с 9100 на 9200 ):  

![002.png](img/002.png)
* ( слайд вставлен взамен ошибочного ). 
* Оба способа при частом изменении/добавлении опций к node_exporter будут неудобны.
* Используя например команду `systemd-run`, можем с легкостью передавать опции в node_exporter ( пример запуска как transient unit ):  
* ![003.png](img/003.png)
* ( здесь я предварительно сделал `systemctl stop node_exporter` )
* В моем модуле изначальный тип был не simple, поэтому в моем случае для запуска transien unit: `sudo systemd-run -p Type=exec node_exporter --web.listen-address=":9200"`
* Однако и этот способ имеет недостатки: мы потеряем заданные опции при перезагрузке системы, так как используем transient unit.
* Также для запуска разных инстансов юнита можно использовать механизм шаблонов.
* Шаблон:  
```bash
...
WantedBy=multi-user.target
```
* На скрине демонстрирую, что запущен инстанс шаблона node_exporter@.service с опцией включить метрику WiFi:
* ![004.png](img/004.png)
* Однако, чтобы таким способом сделать опцию постоянной, нужно будет создать target Unit для запуска инстанса после перезагрузки:
* Содержимое такого target'a может быть:
* [Unit]
* Wants=node_exporter@--collector.wifi
* [Install]
* WantedBy=multi-user.target
* после того, как мы сделаем enable для нового target'a, инстанс станет запускаться сам.
